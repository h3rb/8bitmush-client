...
{
{
float4 _colour = gl_texture2D(_gm_BaseTexture, _v_vTexcoord);
float _lum = (((_colour.x + _colour.y) + _colour.z) / 3.0);
(gl_Color[0] = (vec4(_lum, _lum, _lum, _colour.w) * _v_vColour));
}
}
;
struct PS_INPUT
